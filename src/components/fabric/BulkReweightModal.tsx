        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[100] flex items-center justify-center p-2 sm:p-4">
                ...
                <div className="p-4 sm:p-5 border-b border-border flex items-center justify-between bg-secondary/10 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-white/60 font-bold uppercase tracking-widest mt-0.5">Define correction rules per category</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <button
                            onClick={() => setShowReport(true)}
                            className="flex items-center gap-2 px-4 py-2 bg-secondary/40 text-foreground border border-border rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-secondary transition-all"
                        >
                ...
                <div className="px-6 py-4 bg-secondary/5 border-b border-border flex items-center justify-between gap-4 shrink-0">
                    <div className="flex items-center gap-5">
                        ...
                        <div className="flex bg-secondary/30 p-1.5 rounded-xl border border-border w-[380px]">
                            <button onClick={() => setAdjustmentMode('distribute')} className={`flex-1 py-2.5 text-[10px] font-black uppercase rounded-lg transition-all ${adjustmentMode === 'distribute' ? 'bg-primary text-white shadow-md' : 'text-white/40 hover:text-white/60'}`}>Global Target</button>
                            <button onClick={() => setAdjustmentMode('adjust')} className={`flex-1 py-2.5 text-[10px] font-black uppercase rounded-lg transition-all ${adjustmentMode === 'adjust' ? 'bg-primary text-white shadow-md' : 'text-white/40 hover:text-white/60'}`}>Rule Based</button>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-start">
                        ...
                        <div className="lg:col-span-1 space-y-5">
                            <div className="space-y-4">
                                <span className="text-xs font-black uppercase tracking-widest text-white/50 italic">1. Global Filter</span>
                            ...
                            <div className="space-y-4 pt-4 border-t border-dashed border-border">
                                <span className="text-xs font-black uppercase tracking-widest text-white/50 italic">2. Identity</span>
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        className="w-full px-4 py-3 bg-secondary/40 border-2 border-primary/10 rounded-xl text-xs font-bold placeholder:text-white/20 focus:border-primary outline-none transition-all text-foreground"
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="lg:col-span-2 space-y-5">
                            <span className="text-xs font-black uppercase tracking-widest text-white/50 italic">3. Weight Details</span>
                            {adjustmentMode === 'distribute' ? (
                                <div className="bg-secondary/10 p-5 rounded-2xl border border-border space-y-6 shadow-inner">
                                    <div className="space-y-4">
                                        <div className="flex bg-secondary/30 p-1 rounded-xl border border-border">
                                            <button onClick={() => setDistAction('offset')} className={`flex-1 py-2 text-[10px] font-black uppercase rounded-lg transition-all ${distAction === 'offset' ? 'bg-primary text-white shadow-md' : 'text-white/40 hover:text-white/60'}`}>ADJUSTMENT (+/-)</button>
                                            <button onClick={() => setDistAction('replace')} className={`flex-1 py-2 text-[10px] font-black uppercase rounded-lg transition-all ${distAction === 'replace' ? 'bg-primary text-white shadow-md' : 'text-white/40 hover:text-white/60'}`}>FIXED TOTAL</button>
                                        </div>
                                        <div className="space-y-2">
                                            ...
                                            <p className="text-[9px] text-white/30 font-bold uppercase italic px-1">
                                    ...
                                    <div className="flex flex-col gap-2">
                                        {['equal', 'proportional', 'weight'].map((type) => (
                                            <button key={type} onClick={() => setDistributionType(type as any)} className={`px-5 py-3 rounded-lg border-2 text-[10px] font-black uppercase transition-all flex items-center justify-between ${distributionType === type ? 'border-primary bg-primary text-white shadow-md' : 'border-border text-white/40 bg-secondary/40 hover:text-white/60'}`}>
                                ...
                                <div className="space-y-3 max-h-[450px] overflow-y-auto custom-scrollbar pr-1 min-h-[200px]">
                                    {adjustmentRules.map((rule) => (
                                        <div key={rule.id} className="p-2 sm:p-3 bg-secondary/40 rounded-xl border border-border shadow-sm border-l-[6px] border-l-primary/30 flex items-center gap-2">
                                            <select value={rule.dia} onChange={(e) => updateRule(rule.id, 'dia', e.target.value)} className="flex-1 min-w-0 px-2 py-2 bg-slate-950/50 border border-border rounded-lg text-[10px] font-bold focus:outline-none text-foreground">
                                                <option value="all" className="bg-slate-900">Any DIA</option>
                                                {uniqueDias.map(dia => <option key={dia} value={dia} className="bg-slate-900">{dia} DIA</option>)}
                                            </select>
                                            <select value={rule.color} onChange={(e) => updateRule(rule.id, 'color', e.target.value)} className="flex-1 min-w-0 px-2 py-2 bg-slate-950/50 border border-border rounded-lg text-[10px] font-bold focus:outline-none text-foreground">
                                                <option value="all" className="bg-slate-900">Any Color</option>
                                                {uniqueColors.map(c => <option key={c} value={c} className="bg-slate-900">{c}</option>)}
                                            </select>
                                            <select value={rule.distType} onChange={(e) => updateRule(rule.id, 'distType', e.target.value)} className="flex-[1.4] min-w-0 px-2 py-2 bg-slate-950/50 border border-border rounded-lg text-[10px] font-black uppercase italic focus:outline-none text-foreground">
                                                <option value="per_item" className="bg-slate-900">Per Roll</option>
                                                <option value="total_equal" className="bg-slate-900">Equal Split</option>
                                                <option value="total_pcs" className="bg-slate-900">Split PCS</option>
                                                <option value="total_weight" className="bg-slate-900">Split Weight</option>
                                            </select>
                                            <input type="number" value={rule.value} onChange={(e) => updateRule(rule.id, 'value', e.target.value)} placeholder="KG" className="w-16 px-2 py-2 bg-slate-950/50 border-2 border-primary/20 rounded-lg text-xs font-black text-center focus:border-primary outline-none text-foreground" />
                                    ...
                                    <button onClick={addRule} className="w-full py-4 border-2 border-dashed border-primary/30 rounded-xl text-primary text-xs font-black uppercase bg-primary/5 hover:bg-primary/10 transition-colors">+ Add Optimization Rule</button>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="lg:col-span-1 bg-secondary/10 rounded-[1.5rem] border border-border overflow-hidden flex flex-col max-h-[500px]">
                            <div className="p-3 border-b border-border bg-secondary/20 flex items-center justify-between shrink-0">
                                <span className="text-xs font-black uppercase tracking-widest text-white/50">4. Impact Preview</span>
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                                {previews.map((p, i) => (
                                    <div key={i} className={`p-3 rounded-xl border transition-all flex flex-col gap-2 ${p.changed ? 'bg-secondary/50 border-primary shadow-sm scale-[1.01]' : 'bg-secondary/10 border-border/10 opacity-60'}`}>
                                        <div className="flex items-center justify-between">
                                            <div className="text-[10px] font-black text-white/30 uppercase">{p.color} • {p.diameter}"</div>
                                        ...
                                        <div className="flex items-center justify-between leading-none">
                                            {p.changed ? (
                                                ...
                                                <span className="text-xs font-bold text-white/40">{Number(p.quantity).toFixed(2)} KG</span>
                                            )}
                                        </div>
                                    </div>
                                ))}
                    ...
                    <div className="flex items-center gap-8 z-10">
                        <div className="flex gap-10">
                            <div className="space-y-1">
                                <span className="text-[10px] font-black text-white/40 uppercase tracking-widest">Base Weight</span>
                                <div className="text-base font-black text-white/60">{totalOldWeight.toFixed(2)} KG</div>
                            </div>
                            <div className="w-px h-10 bg-border" />
                            <div className="space-y-1">
                                <span className="text-[10px] font-black text-white/40 uppercase tracking-widest">Net Correction</span>
                                <div className={`text-xl font-black leading-none ${netCorrection > 0 ? 'text-blue-500' : netCorrection < 0 ? 'text-red-500' : 'text-white/30'}`}>
                            ...
                            <div className="space-y-1">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className={`px-2 py-0.5 rounded text-[8px] font-black ${netCorrection > 0 ? 'bg-blue-600 text-white' : netCorrection < 0 ? 'bg-red-600 text-white' : 'bg-secondary text-white/50'}`}>
                        ...
                        <button
                            ...
                            className="w-full md:w-auto px-14 py-5 bg-[#ff4d00] text-white rounded-2xl font-black text-[13px] uppercase tracking-[0.2em] shadow-2xl shadow-[#ff4d00]/40 hover:scale-[1.05] hover:-translate-y-1 hover:brightness-125 active:scale-[0.95] disabled:bg-[#ff4d00]/40 disabled:text-white/40 disabled:shadow-none disabled:scale-100 disabled:translate-y-0 transition-all duration-500 z-10 flex items-center justify-center gap-4 border border-white/10"
                        >
            ...
            {showReport && (
                <div className="fixed inset-0 z-[110] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-white w-full max-w-2xl max-h-[90vh] sm:max-h-[85vh] rounded-3xl overflow-hidden flex flex-col shadow-2xl animate-in fade-in zoom-in duration-300 text-black">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 sm:p-8 bg-white custom-scrollbar" id="reweight-report-content">
                            <div className="border-[2px] sm:border-[3px] border-black p-4 sm:p-8 space-y-6 sm:space-y-8 min-w-[500px] sm:min-w-0">
                                <div className="flex items-center justify-between border-b-[3px] border-black pb-6">
                                    <div>
                                        <h1 className="text-3xl font-black uppercase tracking-tighter text-black">REWEIGHT STATEMENT</h1>
                                        <p className="text-[10px] font-bold text-black/50 tracking-[0.3em] uppercase mt-1">Quality Control Department</p>
                                    </div>
                                    <div className="text-right">
                                        <div className="text-[10px] font-black uppercase text-black/40">Date Generated</div>
                                        <div className="text-sm font-bold text-black">{new Date().toLocaleDateString()} {new Date().toLocaleTimeString()}</div>
                                    </div>
                                </div>

                                <div className="grid grid-cols-2 gap-8 text-[11px]">
                                    <div>
                                        <div className="text-[9px] font-black text-black/40 uppercase mb-1">Authenticated By</div>
                                        <div className="text-lg font-black uppercase tracking-tight text-black">{reweightedBy || 'Not Specified'}</div>
                                    </div>
                                    <div className="text-right">
                                        <div className="text-[9px] font-black text-black/40 uppercase mb-1">Batch Status</div>
                                            ...
                                            {previews.map((p, i) => {
                                                ...
                                                return (
                                                    <tr key={i} className={`border-b-[2px] border-black ${!p.changed ? 'bg-gray-50/50 opacity-80' : ''}`}>
                                                        ...
                                                        <td className={`p-3 font-black ${p.changed ? 'text-black bg-gray-50' : 'text-black/30'}`}>{Number(p.newWeight).toFixed(2)}</td>
                                                    </tr>
                                                );
                                            })}
                                        </tbody>
                                ...
                                <div className="pt-20 flex justify-between text-black">
